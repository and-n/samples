...
public class Ce102Impl implements Ce102 {
	...
	DataTypesInputStream din = new DataTypesInputStream(
		new ByteArrayInputStream(buf));
	Calendar c = Calendar.getInstance(getTimeZone());
	c.set(Calendar.SECOND, din.readBCD());
	c.set(Calendar.MINUTE, din.readBCD());
	c.set(Calendar.HOUR_OF_DAY, din.readBCD());
	c.set(Calendar.DAY_OF_WEEK, din.readBCD()+1);
	c.set(Calendar.DAY_OF_MONTH, din.readBCD()-1);
	...
	dout.writeBCD(c.get(Calendar.DAY_OF_WEEK-1));
	dout.writeBCD(c.get(Calendar.DAY_OF_MONTH));
	dout.writeBCD(c.get(Calendar.MONTH)+1);
	...
	return new BigDecimal(1).scaleByPowerOfTen(
		-1 * ((int) getConfig() & 0x03)).doubleValue();
    }
	...
	dout.writeBCD(num);
	dout.writeBCD(count);
	byte[] buf = requestPal(CommandEnum.ReadIntervalEnergy,
		bout.toByteArray());
	ByteArrayInputStream bin = new ByteArrayInputStream(buf);
	DataTypesInputStream din = new DataTypesInputStream(bin);
    ...
    protected byte[] requestPal(CommandEnum cmd, byte[] data)
	    throws IOException, AccessException, InterruptedException {
	IPhysicalChannel ch = PhysicalChannelManager.getInstance()
		.captureChannel(getChannelParameters());
	try {
	    ch.setReadTimeout(3000);
	    ch.open();
    ...
    @SuppressWarnings("resource")
    private PALResponse sendAndWait(PALRequest req, IPhysicalChannel ch)
	    throws IOException {
	ByteArrayOutputStream bout = new ByteArrayOutputStream();
	...
	cout.writePacket(new ChannelLevelPacket(getAddress(), 253, 0x48, bout
		.toByteArray()));
	cout.flush();
	ChannelLevelPacket chPack = cin.readPacket();
	if (chPack.getAddressD() != 253 || chPack.getAddressS() != getAddress())
	    throw new IOException("addressD <> addressS, "+chPack.getAddressS());
	PALInputStream pin = new PALInputStream(new ByteArrayInputStream(
		chPack.getPacketData()));
